...
def export_spreadsheet_from_csv(csv_path, tournament_name, cs, logo, logo_path, zoom_logo, date, spreadsheet_pixel_density):
    ...
    # Fixed dimensions
    row_height = 0.3  # inches
    col_width = 0.8   # inches
    
    fig_height = (len(rows) + 1) * row_height
    fig_width = len(headers) * col_width

    # Use add_axes to force the axes to occupy the full figure
    ...
    # --- Crop exactly to the table bbox ---
    fig.canvas.draw()
    ...
    (bbox.width + 2*margin) / bbox.width,
    (bbox.height + 2*margin) / bbox.height
    )
    
    # Save image cropped tightly to the table
    fig.savefig(
